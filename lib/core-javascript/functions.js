...
// and the value of the new function's first argument.
function curriedAdd() {}
...
function callGivenFunctionWithGivenContextAndArguments(
  functionToCall,
  context
) {}
...
function returnAFunctionThatIsBoundToTheGivenContextAndArgument(
  functionToCall,
  context,
  argument
) {}
...
function returnAFunctionThatIsBoundToTheGivenContextAndArguments(
  functionToCall,
  context
) {}
...
// This is the same exercise as above, but you are not allowed to use a recursive function.
// (Yes, it is expected to be harder and look much less terse!)
function countNodesWithoutFunctionRecursion(node) {}

// This function takes in a node. Nodes are of the format {nodes: [], action: Function },
// where the nodes array can contain more nodes, and in turn, those nodes can contain more nodes.
// This represents a "tree-like" structure. The executeFunctionsInReverseOrderAndReverseDepth
// function should iterate over the child nodes of the current node in reverse order, executing their action functions
// and then finally execute the current node's action function.
...
// ----D
// ----E
// the function should execute the action on E, then D, then C, then B, then A.
function executeFunctionsInReverseOrderAndReverseDepth(node) {}
...
module.exports = {
  ...
  countNodes,
  countNodesWithoutFunctionRecursion,
  executeFunctionsInReverseOrderAndReverseDepth
};
